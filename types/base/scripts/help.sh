...
- \`clean\`: the reciprocal of the \`build\` target. Remove all built
  ...
  - docker containers
  - docker images
  Keep in mind it executes the target \`down\` as a prerequisite.
...
- \`inspect\`: Inspect a given \`instance\` configuration. If something is
  wrong in the way the \`instance\` is configured, an error message will
  output. Otherwise, a \`yaml\` formatted configuration will be output.
- \`nuke\`: As dangerous as the name suggests. In essence, this is the same
  thing as \`clean\`. However, persistent volume attached to this \`instance\`
  will also be deleted. You'll lost everything of this \`instance\`. You've
  been warned, uniquely viable for mushrooms hardcore fans.
EOF
